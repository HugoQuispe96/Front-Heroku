<template>
  <div id="layout">
    <!--Tarjeta de presentacion de adelanto de crecimiento -->
    <br />
    <div
      ...
      :key="adelanto._id"
    >
      <div id="imagen_estado" v-if="adelanto.estado == true" style="background:#42f435" >
        <v-img id="logo" :src="adelanto.imagen"></v-img>
      </div>
      <div id="imagen_estado" v-if="adelanto.estado == false" style="background: #f44235">
        <v-img id="logo" :src="adelanto.imagen"></v-img>
      </div>
      
      <div id="derecha">
        <p id="titulo">{{adelanto.nombre}}</p>
        <p id="sub_titulo">{{adelanto.estado}}</p>
      </div>
      <br />
      <!------------------------------------------------------------------------------------------------------------ -->
      <v-card-actions>
          ...
          <div
            ...
            :key="requisito._id">
              ...
              <div id="color_estado" v-if="requisito.estado == 'Pendiente'" style="background: #f49f35"></div>
                ...
                <hr>
                <!--BOTONES INTEGRANTE-->
                <v-btn v-if="requisito.estado == 'No realizado' && rol == 'Integrante'" id="boton1" text v-on:click="peticion_integrante(requisito._id, adelanto.tipo)">Pedir validación</v-btn>
                <v-btn v-else-if="requisito.estado == 'Pendiente' && rol == 'Integrante'" id="boton1" text v-on:click="peticion_integrante(requisito._id, adelanto.tipo)">Cancelar petición</v-btn>
                <!--BOTONES JEFE-->
                <!--BOTONE 1-->
                <v-btn v-else-if="requisito.estado == 'No realizado' && rol != 'Integrante'" id="boton1" text v-on:click="realizado_a_no_realizado(requisito._id, adelanto.tipo)">Aceptar</v-btn>
                <v-btn v-else-if="requisito.estado == 'Realizado' && rol != 'Integrante'" id="boton1" text v-on:click="realizado_a_no_realizado(requisito._id, adelanto.tipo)">Cancelar</v-btn>
                <v-btn v-else-if="requisito.estado == 'Pendiente' && rol != 'Integrante'" id="boton1" text v-on:click="rechazar_peticion(requisito._id, adelanto.tipo)">Rechazar petición</v-btn>
                <!--BOTONE 2-->
                <v-btn v-if="requisito.estado == 'Pendiente' && rol != 'Integrante'" id="boton2" text v-on:click="aceptar_peticion(requisito._id, adelanto.tipo)">Aceptar petición</v-btn>
              </div>
              <br>
              <br>
          </div>
